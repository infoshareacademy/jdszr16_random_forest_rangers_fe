...
const onSubmit = async () => {
  ...
  return (
      <div>
          <span style={{fontSize: 14, marginRight: 20}}> Dlugość opisu o chorobie (liczba zdań) <InputNumber min={5} max={30}
                                                                                          defaultValue={5}
                                                                                          onChange={onLengthChange}/></span>
          <Checkbox onChange={onChecboxChange}>Jestem lekarzem</Checkbox>
          <Input style={{width: 300, marginLeft: 20, marginRight: 20}} placeholder="Wpisz chorobe (tylko dla testow)"
                 onChange={onDiseaseChange}/>
          <Button type="primary" onClick={onSubmit}>Submit</Button>

          <div> <TextArea rows={10} style= {{width: 1000, marginTop: 20}} value = {diseaseDescription} placeholder = 'Informacje o chorobie'/></div>
